...
public class OrderService {
    ...
    public OrderResponse confirmCashPayment(Long orderId) {
        ...
        if (!order.getUser().getId().equals(user.getId())) {
            throw new RuntimeException("No tienes permisos para confirmar este pedido");
        }

        // Verificar que el método de pago es efectivo
        if (order.getPaymentMethod() != Order.PaymentMethod.EFECTIVO) {
            throw new RuntimeException("Este pedido no es de pago en efectivo");
        }
        ...
        // Confirmar el pago cambiando el estado a CONFIRMADO
        order.setStatus(Order.OrderStatus.CONFIRMADO);
        Order savedOrder = orderRepository.save(order);
